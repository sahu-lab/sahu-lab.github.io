...
# ╔═╡ 5fd8bab4-576a-42ef-9ee6-cef314247d3e
begin
	using Plots
	using PlutoUI
	using LinearAlgebra
...
For completeness, we write the $j = 4$ equation as well:
```math
C_5
\, - \, \gamma \, C_4
\, + \, C_3
...
~,
```
where $C_5$ is known from the boundary condition.
These are the four equations required to solve for the four unknowns $C_1$, $C_2$, $C_3$, and $C_4$.


"""

# ╔═╡ ff8222b8-df81-4298-ac42-4f140107792a
md"""
### Simplification to a linear system

Since the four equations are linear, we can express them in terms of vectors and matrices as
```math
\boldsymbol{A} \, \boldsymbol{c}
\, = \, \boldsymbol{b}
~,
```
where $\boldsymbol{A}$ is a $4 \times 4$ matrix of constant coefficients, $\boldsymbol{c}$ is a $4 \times 1$ vector of the unknown concentrations, and $\boldsymbol{b}$ is a constant vector which (as we will see) contains the boundary conditions.
For now, let us expand the above equation as
```math
\begin{bmatrix}
\, ? & ? & ? & ? \, \\[4pt]
\, ? & ? & ? & ? \, \\[4pt]
...
~,
```
where we would like to fill in the unknown entries.
Recall that the first equation, where $j = 1$, is given by $C_2 - \gamma C_1 + C_0 = 0$, where $C_0 = 1$ from the bottom boundary condition.
Thus, we have
```math
C_2
\, - \, \gamma \, C_1
...
!!! question "Numerical Question"
	How would you enter this information into the above system of equations?
	Remember, this is the first equation, so we would like to enter it into the first row.
...
~.
```
Now recall the second equation, which is reproduced here as
```math
C_3
\, - \, \gamma \, C_2
\, + \, C_1
...
!!! question "Numerical Question"
	Try to enter this equation into the above matrix--vector equation!
```math
\begin{bmatrix}
-\gamma & 1 & 0 & 0 \, \\[4pt]
\, 1 & -\gamma & 1 & 0 \, \\[4pt]
...
~.
```
Entering it into the matrix equation is left as an exercise.
Finally, the fourth equation is written as $C_5 - \gamma C_4 + C_3 = 0$, but we know $C_5 = \beta$ from the top boundary condition.
Thus, we can write the final equation as
```math
- \, \gamma \, C_4
\, + \, C_3
\, = \, - \beta
~.
```
We enter this into our matrix system as
```math
\begin{bmatrix}
-\gamma & 1 & 0 & 0 \, \\[4pt]
\, 1 & -\gamma & 1 & 0 \, \\[4pt]
...
~.
```
We accordingly have a linear system of equations, and the concentrations $\boldsymbol{c}$ can be solved for according to
```math
\boldsymbol{c}
\, = \, \boldsymbol{A}^{-1} \boldsymbol{b}
~.
```
As mentioned earlier, notice how the vector $\boldsymbol{b}$ contains all of the information about the boundary conditions!



"""

# ╔═╡ 5eb307d1-0a01-4867-a947-c2c59a11fcba
md"""
### Numerical implementation

Before starting our numerical implementation, we would like to compare with the analytical solution.
You should be able to solve the differential equation, and can do so as practice.
Here, we provide a function that returns the analytical solution:

"""

# ╔═╡ 9d228a74-57c1-4ffc-8456-94fccc249163
function slab_exact(z::Float64, alpha::Float64, beta::Float64)
	sqa = sqrt(alpha)
	return ( (beta - exp(-sqa)) * exp(sqa * z) + (exp(sqa) - beta) * exp(-sqa * z) ) / ( exp(sqa) - exp(-sqa) )
end

# ╔═╡ 73f515ea-ceda-4126-9682-0c54f73d79fa
md"""
!!! question "Practice Question"
	At this point, you should be able to see the above function, and from it be able to write down the analytical expression on paper!
...
md"""
!!! question "Conceptual Question"
	Given that we have the analytical solution in this case, which is a function, how would you calculate the error in the numerical solution?
